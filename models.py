...
def segmentationResnet50(num_classes):
    return smp.Unet('resnet18', in_channels = 3, classes=num_classes, activation='softmax')
    

def deeplabv3(num_classes ):
    model = torchvision.models.segmentation.deeplabv3_mobilenet_v3_large(weights=None)
    #model.classifier[4] = torch.nn.Conv2d(256, 16, kernel_size=(1, 1), stride=(1, 1)) # Change final layer to 3 classes
    #model.classifier[3] = torch.nn.Softmax()
...
def LRASPP_MOBILENET(num_classes):
    return torchvision.models.segmentation.lraspp_mobilenet_v3_large(num_classes)


def deeplabv3PlusR50(num_classes ):
    model = smp.DeepLabV3Plus('resnet50', in_channels = 3, classes=num_classes, activation='softmax', encoder_weights='imagenet')
    #model.classifier[4] = torch.nn.Conv2d(256, 16, kernel_size=(1, 1), stride=(1, 1)) # Change final layer to 3 classes
    #model.classifier[3] = torch.nn.Softmax()
